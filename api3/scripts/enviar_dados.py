"""Envia novos eventos do sensor de trânsito e mede cada POST."""
...
JSON_FILE = ROOT / "jsons" / "transito_transporte.json"
...
for indice in range(args.amostras):
    ...
    request = Request(f"http://127.0.0.1:3003/api/v1/sensores/{sensor}/eventos", data=json.dumps(payload, ensure_ascii=False).encode(), method="POST", headers={"Content-Type": "application/json", "X-Source-Id": sensor})
    inicio = time.perf_counter()
    with urlopen(request, timeout=5) as response:
        print(f"HTTP {response.status} | {(time.perf_counter()-inicio)*1000:.2f} ms | {response.read().decode()}")
with urlopen("http://127.0.0.1:3003/api/v1/eventos?limite=1", timeout=5) as response:
    print("Último evento persistido:", response.read().decode())
