...
JSON_FILE = ROOT / "jsons" / "qualidade_ar.json"
SUFFIX = "leituras"
...
def main():
    ...
    for indice in range(args.amostras):
        ...
        request = Request(f"http://127.0.0.1:3001/api/v1/sensores/{sensor}/{SUFFIX}", data=body, method="POST", headers={"Content-Type":"application/json", "X-Source-Id":sensor})
        inicio = time.perf_counter()
        with urlopen(request, timeout=5) as response:
            print(f"HTTP {response.status} | {(time.perf_counter()-inicio)*1000:.2f} ms | {response.read().decode()}")
    with urlopen("http://127.0.0.1:3001/api/v1/leituras?limite=1", timeout=5) as response:
